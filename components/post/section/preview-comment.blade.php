        ...
        @if ($topComment['files']['images'])
            <div class="d-flex align-content-start flex-wrap comment-image-{{ count($topComment['files']['images']) }}">
                @foreach($topComment['files']['images'] as $image)
                    <a class="position-relative image-box" data-fancybox="comment-{{ $topComment['cid'] }}" data-src="{{ $image['imageBigUrl'] }}">
                        <img src="{{ $image['imageSquareUrl'] }}" loading="lazy" class="img-fluid">
                    </a>
                    <img src="{{ $image['imageSquareUrl'] }}" data-fancybox="comment-{{ $topComment['cid'] }}" loading="lazy" class="img-fluid zoom-image">
                @endforeach
            </div>
        @endif
    </section>
